...
public class Verifica {
    ...
    private static boolean existeNotificacionFechaAanterior= false;
    private static boolean existeNotificacionFechaMactual= false;
...
//        boolean encontros = IntStream.of(TIPOS_DOCUMENTO).anyMatch(element -> element == tipo_doc);
    ...
    public static void verificarCodigoConcepto(String codigoConcepto, int index, List<ErrorTransaccion> listErrores) {
        ...
        if (esNumerico(codigoConcepto)) {
            int numero = Integer.parseInt(codigoConcepto);

            if (numero < 1000 || numero > 2799) {
                listErrores.add(new ErrorTransaccion(index, "Concepto fuera de rango, debera ser > 000 y < 2799"));
            }
    ...
    public static void verificarDescripcionConcepto(String descripcionConcepto, int index, List<ErrorTransaccion> listErrores) {
        ...
        // Verificar caracteres no deseados (ejemplo: números y caracteres especiales)
        if (contieneCaracteresNoDeseados(descripcionConcepto)) {
            listErrores.add(new ErrorTransaccion(index, "La descripción contiene caracteres no permitidos."));
        }
    }

    private static boolean contieneCaracteresNoDeseados(String descripcionConcepto) {
        // Definir la lista de caracteres no deseados (puedes ajustarla según tus necesidades)
        String caracteresNoDeseados = "@#$%^&*()?";

        // Verificar si la descripción contiene algún carácter no deseado
        for (char c : descripcionConcepto.toCharArray()) {
            if (caracteresNoDeseados.contains(String.valueOf(c))) {
                return true;
            }
        }
    ...
    public static void verificarCantidad(String cantidad, int index, List<ErrorTransaccion> listErrores) {
        ...
        // Verificar si el importe contiene el carácter ';'
        if (cantidad.contains(";")) {
            listErrores.add(new ErrorTransaccion(index, "El importe tiene que tener un '.' para los decimales"));
        } else {
    ...
    public static void verificarImporte(String importe, int index, List<ErrorTransaccion> listErrores) {
        ...
        Matcher matcher = patron.matcher(importe);
        // Verificar si el importe contiene el carácter ';'
        if (!matcher.matches()) {
            listErrores.add(new ErrorTransaccion(index, "El importe posee un formato no valido"));
        } else {
            if (!esCantidadCero) {
    ...
    public static void verificarFecha(String fecha, int index, List<ErrorTransaccion> listErrores) {
        ...
        if (esNumerico(fecha)) {
            ...
            String msj = "Fecha invalida, Respete el siguiente Formato: 'AÑO/MES/DIA'";
            listErrores.add(new ErrorTransaccion(index, msj));
        }
    }
    ...
    private static String obtenerFechaArchivo(String fecha, int index, List<ErrorTransaccion> listErrores) {
        ...
        if (Integer.parseInt(anio) < anioActual  && !existeNotificacionFechaAanterior) {
            NotificacionList.getInstance().agregar(msj.añoAnterior());
//            Mensajes.advertencia(msj.añoAnterior());
            existeNotificacionFechaAanterior = true;
        }
        
        
       
        if ((Integer.parseInt(mes) > mesActual) && (anioActual > Integer.parseInt(anio)) && !existeNotificacionFechaMactual) {
          NotificacionList.getInstance().agregar(msj.fechaMayorActual());
//            Mensajes.advertencia(msj.fechaMayorActual());
            existeNotificacionFechaMactual = true;
    ...
    private static void verificarSignoComaCantidad(String numero_sinSigno, String cantidad, int index, List<ErrorTransaccion> listErrores) {
        ...
        if (longitud <= LONGITUD_CANTIDAD) {
            ...
            if (!esCantidadNegativa) {
                if ((cantidad.equals("0,00")) || (cantidad.equals("0")) || (cantidad.equals("0,0"))) {
                    esCantidadCero = true;
                }
            }
        } else {
