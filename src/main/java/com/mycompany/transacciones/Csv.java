...
// Método para leer el archivo, omitiendo la cabecera si existe
    ...
    public boolean isValid() {
        int filaNumero = 1;
        for (String[] fila : r) {
            if (fila.length != TOTAL_COLUMNAS_ARCHIVO) {
                String error = "Error en fila " + filaNumero + " \n " + Arrays.toString(fila);
    ...
    private boolean tieneCabecera() throws IOException, CsvValidationException {
        try (CSVReader reader = new CSVReaderBuilder(
            ...
            if (primeraLinea != null) {
                if (primeraLinea[0].trim().length() != 2) {
                    //  System.out.println("tiene cabecer 1° cond "+ "leng "+primeraLinea[0].length());
                    return true;

                }
                if (primeraLinea[1].trim().length() != 2) {
                    //  System.out.println("tiene cabecera 2° ");
                    return true;
                }
